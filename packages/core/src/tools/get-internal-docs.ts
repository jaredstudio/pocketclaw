...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find PocketClaw documentation directory.');
}
  ...
  async execute(_signal: AbortSignal): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available PocketClaw documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to PocketClaw's internal documentation.
