...
import { getPackageJson, debugLogger } from '@pocketclaw/core';
  ...
  try {
    ...
    if (isNightly) {
      ...
      if (bestUpdate && semver.gt(bestUpdate, currentVersion)) {
        const message = `A new version of PocketClaw is available! ${currentVersion} → ${bestUpdate}`;
      ...
      if (latestUpdate && semver.gt(latestUpdate, currentVersion)) {
        const message = `PocketClaw update available! ${currentVersion} → ${latestUpdate}`;
